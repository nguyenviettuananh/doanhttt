...
module.exports = function (controller, component, app) {
    ...
    let baseRoute = '/admin/quiz/categories/';
    ...
    controller.categoryQuickCreate = function (req, res) {
        app.feature.category.actions.create(req.body, 'quiz').then(function () {
    ...
    controller.categorySave = function (req, res, next) {
        ...
        app.feature.category.actions.create(data, 'quiz').then(function (category) {
            req.flash.success(__('m_category_backend_category_flash_save_success'));
    ...
    controller.categoryDelete = function (req, res) {
        ...
        Promise.all([
            // Update quizzes have categories was deleted
            Promise.map(listId, function (id) {
                return app.models.quiz.findAndCountAll({
                    where: {
                        categories: {
                            $like: '%:' + id + ':%'
                        }
                    ...
                    if (quizzes.count > 0) {
                        return Promise.map(quizzes.rows, function (quiz) {
                            let oldCategory = quiz.categories;
                            let newCategory = '';

                            // If the quiz has multiple categories, remove deleted category from string
                            if (oldCategory != (':' + id + ':'))
                                newCategory = oldCategory.replace(':' + id + ':', ':');

                            return quiz.updateAttributes({
